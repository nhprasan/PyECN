...
#        self.__dict__.update(params_update)      #update the unique inputs for this 'self'        
        ...
        self.tab_W = 2*self.weld_W1 + self.weld_W2*(self.weld_n-1) + self.weld_W3*self.weld_n
        ...
        if self.pos_neg == 'pos':
            self.rou_c_V_weights = (self.tab_rouXc_pos * self.V_4T).reshape(-1,1)             #prep for fun_weighted_avg_and_std
        elif self.pos_neg == 'neg':
            self.rou_c_V_weights = (self.tab_rouXc_neg * self.V_4T).reshape(-1,1)             #prep for fun_weighted_avg_and_std
        self.fun_get_Geo_4T()
    ...
    def fun_matrix1(self):
        ...
        xi = (xn-1) * self.tab_L1/self.tab_nx        
        ...
        yi = np.repeat(yi,self.nx*self.nz)
        
        
        zi=(zn-1)*self.tab_T1/(self.nz)    
        ...
        #---------------------------- volume ---------------------------------
        # tab nodes evenly divided in length and thickness directions, but unevenly in width (y) direction
        
        Lx = np.ones(self.ntotal_4T) * self.tab_L1/self.tab_nx
                
        Ly = np.zeros([self.ny])
        Ly_nonweld = self.weld_W2 * np.ones([self.nonweld_n])
        Ly_nonweld[0] = self.weld_W1
        Ly_nonweld[-1] = self.weld_W1
        Ly_weld = self.weld_W3 * np.ones([self.weld_n])
        Ly[2*np.arange(self.nonweld_n)] = Ly_nonweld
        Ly[2*np.arange(self.weld_n)+1] = Ly_weld
        Ly = np.repeat (Ly,self.nz*self.nx)
        
        Lz = np.ones(self.ntotal_4T) * self.tab_T1 /self.tab_nz
...
#        global T3_4T_ALL, ind0_BCtem_ALL, ind0_BCtem_others_ALL, h_4T_ALL, Tconv_4T_ALL, ind0_BCconv_ALL, ind0_BCconv_others_ALL
    ...
    #=============================convection BC input
            ...
            h_4T[ self.ind0_Geo_left_4T,0 ]= 30                                                                                                                  
            
            h_4T[ self.ind0_Geo_right_4T,1 ]= 30                                                                                                                  
            ...
            Tconv_4T[ self.ind0_Geo_left_4T,0 ]= self.T_initial                                                                             
                             
            Tconv_4T[ self.ind0_Geo_right_4T,1 ]= self.T_initial                                                                             
        ...
        if self.status_BC_tab=='OneSurfaceTabCooling':
    #=============================temperature BC input
            #-----------------------------input: T and its coresponding node
            T3_4T=np.nan*np.zeros([self.ntotal_4T,1])                                                                                                           #T_4T initialization, shape is (63,1)        
    ...
    #=============================convection BC input
            ...
            h_4T[ self.ind0_Geo_back_4T,4 ]=  self.h_tab                                                                          
            ...
            Tconv_4T[ self.ind0_Geo_back_4T,4 ]=  self.T_cooling_tab 

            if ip.status_P_cooling_tab == 'Yes':
                Tconv_4T[ self.ind0_Geo_back_4T,5 ]= self.T_var_cooling_record[self.step]                                                                          

            Tconv_4T[ self.ind0_Geo_front_4T,5 ]= self.T_initial                                                                                               
            #-----------------------------get all constrained node number                
            ind0_BCconv=np.concatenate((self.ind0_Geo_left_4T,self.ind0_Geo_right_4T,self.ind0_Geo_top_4T,self.ind0_Geo_bottom_4T,self.ind0_Geo_back_4T,self.ind0_Geo_front_4T))     #all the convection-constrained nodes
            ind0_BCconv_others=np.array([x for x in np.arange(self.ntotal_4T) if x not in ind0_BCconv],dtype=int) 
        if self.status_BC_tab=='TwoSurfaceTabCooling':
    #=============================temperature BC input
            #-----------------------------input: T and its coresponding node
            T3_4T=np.nan*np.zeros([self.ntotal_4T,1])                                                                                                           #T_4T initialization, shape is (63,1)        
    ...
    #=============================convection BC input
            ...
            h_4T[ self.ind0_Geo_front_4T,5 ]= self.h_tab                                                                                 
            #-----------------------------constrain heat convection temperature: Tconv
            Tconv_4T=np.nan*np.zeros([self.ntotal_4T,6])                                                                                 
            ...
            Tconv_4T[ self.ind0_Geo_front_4T,5 ]= self.T_cooling_tab
            if ip.status_P_cooling_tab == 'Yes':
                Tconv_4T[ self.ind0_Geo_front_4T,5 ]= self.T_var_cooling_record[self.step]                                                                                               
            #-----------------------------get all constrained node number                
            ind0_BCconv=np.concatenate((self.ind0_Geo_left_4T,self.ind0_Geo_right_4T,self.ind0_Geo_top_4T,self.ind0_Geo_bottom_4T,self.ind0_Geo_back_4T,self.ind0_Geo_front_4T))     #all the convection-constrained nodes
            ind0_BCconv_others=np.array([x for x in np.arange(self.ntotal_4T) if x not in ind0_BCconv],dtype=int)     
    ...
    def fun_get_Geo_4T(self):
        ...
        if self.pos_neg == 'pos':
            ind0_Geo_link_4T = np.array([x for x in ind0_Geo_back_4T if x in ind0_Geo_right_4T ],dtype=int)
        elif self.pos_neg == 'neg':
            ind0_Geo_link_4T = np.array([x for x in ind0_Geo_back_4T if x in ind0_Geo_left_4T ],dtype=int)
...
#        global delta_x1_4T, delta_x2_4T, delta_y1_4T, delta_y2_4T, delta_z1_4T, delta_z2_4T, delta_xyz_4T, V_stencil_4T     #for nodes 1~63        
        #-----------------------------------fill in Lamda_4T
        if pos_neg == 'pos':
            Lamda_4T=self.tab_Lamda_pos * np.ones([self.ntotal_4T,6])                                                                                                        #Lamda_4T shape is (63,6). λ term in 6-node stencil
        elif pos_neg == 'neg':
            Lamda_4T=self.tab_Lamda_neg * np.ones([self.ntotal_4T,6])                                                                                                        #Lamda_4T shape is (63,6). λ term in 6-node stencil
        #-----------------------------------fill in RouXc_4T
        if pos_neg == 'pos':
            RouXc_4T=self.tab_rouXc_pos * np.ones([self.ntotal_4T,1])                                                                                                   #RouXc_4T shape is (63,1). ρc term in 6-node stencil
        elif pos_neg == 'neg':
            RouXc_4T=self.tab_rouXc_neg * np.ones([self.ntotal_4T,1])                                                                                                   #RouXc_4T shape is (63,1). ρc term in 6-node stencil
        #-----------------------------------fill in Delta_x1_4T
