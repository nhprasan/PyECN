...
#        self.__dict__.update(params_update)      #update the unique inputs for this 'self'        
        ...
        ####################################
        self.pos_neg = params_update['pos_neg']
        self.weld_W = self.weld_W3*self.weld_n + self.weld_W2*(self.weld_n-1) + 2*self.weld_W1

        self.nx = 1
        self.ny = 2*self.weld_n+1
        self.nz = self.nstack
        ...
        if self.pos_neg == 'pos':
            self.rou_c_V_weights = (self.weld_rouXc_pos * self.V_4T).reshape(-1,1)             #prep for fun_weighted_avg_and_std
        elif self.pos_neg == 'neg':
            self.rou_c_V_weights = (self.weld_rouXc_neg * self.V_4T).reshape(-1,1)             #prep for fun_weighted_avg_and_std
        self.fun_get_Geo_4T()
        self.fun_pre_Thermal(self.pos_neg)
    ...
    def fun_matrix1(self):
        ...
        xn=np.ones([self.ntotal_4T],dtype=int)
        yn=np.tile(yn_unit,self.nz) 
        zn=np.repeat(zn_unit,self.ny)
          
        mat=np.ones([self.ntotal_4T])
        
        xi = np.zeros([self.ntotal_4T])

        yi = np.zeros([self.weld_n+self.nonweld_n])

        yi_nonweld = (self.weld_W2 + self.weld_W3) * np.arange(self.nonweld_n)
        yi_nonweld[1:] -= 0.5*(self.weld_W2-self.weld_W1)
        yi_nonweld[-1] -= 0.5*(self.weld_W2-self.weld_W1)
        
        yi_weld = (self.weld_W2 + self.weld_W3) * np.arange(self.weld_n) + 0.5*(self.weld_W1+self.weld_W3)
        
        yi[2*np.arange(self.nonweld_n)] = yi_nonweld
        yi[2*np.arange(self.weld_n)+1] = yi_weld
        
        yi = np.tile(yi,self.nstack)
        
        zi = np.repeat( (self.weld_T1/self.nstack) * np.arange(self.nstack),self.ny )
        ...
        #---------------------------- volume ---------------------------------
        L = np.zeros([self.ny])
        L_nonweld = self.weld_W2 * np.ones([self.nonweld_n])
        L_nonweld[0] = self.weld_W1
        L_nonweld[-1] = self.weld_W1
        L_weld = self.weld_W3 * np.ones([self.weld_n])
        L[2*np.arange(self.nonweld_n)] = L_nonweld
        L[2*np.arange(self.weld_n)+1] = L_weld
        L = np.tile(L,self.nstack)
        
        V = L * (self.weld_L1*self.weld_T1/self.nstack)        
...
#        global T3_4T_ALL, ind0_BCtem_ALL, ind0_BCtem_others_ALL, h_4T_ALL, Tconv_4T_ALL, ind0_BCconv_ALL, ind0_BCconv_others_ALL
    ...
    #=============================convection BC input
        ...
        h_4T[ self.ind0_Geo_left_right_4T,0 ]= 30                                                                                                                  
                                        
        h_4T[ self.ind0_Geo_left_right_4T,1 ]= 30                                                                                                                  

        h_4T[ self.ind0_Geo_top_4T,2 ]= 30                                                                                      

        h_4T[ self.ind0_Geo_bottom_4T,3 ]= 30                                                                                      

        h_4T[ self.ind0_Geo_back_4T,4 ]= 30                                                                            

        h_4T[ self.ind0_Geo_front_4T,5 ]= 30                                                                                 
        #-----------------------------constrain heat convection temperature: Tconv
        Tconv_4T=np.nan*np.zeros([self.ntotal_4T,6])                                                                                 
    
        Tconv_4T[ self.ind0_Geo_left_right_4T,0 ]= self.T_initial                                                                                                                 
                                        
        Tconv_4T[ self.ind0_Geo_left_right_4T,1 ]= self.T_initial                                                                                                                  

        Tconv_4T[ self.ind0_Geo_top_4T,2 ]= self.T_initial                                                                                     

        Tconv_4T[ self.ind0_Geo_bottom_4T,3 ]= self.T_initial                                                                                      

        Tconv_4T[ self.ind0_Geo_back_4T,4 ]= self.T_initial                                                                           

        Tconv_4T[ self.ind0_Geo_front_4T,5 ]= self.T_initial                                                                                
        #-----------------------------get all constrained node number                
        ind0_BCconv=np.concatenate((self.ind0_Geo_left_right_4T,self.ind0_Geo_top_4T,self.ind0_Geo_bottom_4T,self.ind0_Geo_back_4T,self.ind0_Geo_front_4T))     #all the convection-constrained nodes
    ...
    def fun_get_Geo_4T(self):
        ind0_Geo_front_4T=np.where(self.zn_4T==1)[0]                     
        ind0_Geo_back_4T=np.where(self.zn_4T==self.nz)[0]            
        ind0_Geo_left_right_4T=np.where(self.xn_4T==1)[0]                    
        ind0_Geo_top_4T=np.where(self.yn_4T==1)[0]              
        ind0_Geo_bottom_4T=np.where(self.yn_4T==self.ny)[0]                   

        temp1 = np.tile(self.ny*np.arange(self.nz).reshape(-1,1),(1,self.weld_n))
        temp2 = np.tile((2*np.arange(self.weld_n)+1).reshape(1,-1),(self.nz,1) )
        ind0_Geo_node2_4_6_9_11_13_4T = (temp1 + temp2).reshape(-1)
        temp3 = np.tile(self.ny*np.arange(self.nz).reshape(-1,1),(1,2))
        temp4 = np.tile(np.array([[0,self.ny-1]]),(self.nz,1) )
        ind0_Geo_node1_7_8_14_4T = (temp3 + temp4).reshape(-1)
        ind0_Geo_node1_7_4T = np.array([ x for x in ind0_Geo_node1_7_8_14_4T if self.zn_4T[x]==1 ],dtype=int)
        ind0_Geo_node3_5_10_12_4T = np.array([ x for x in self.node_4T-1 if x not in ind0_Geo_node2_4_6_9_11_13_4T and x not in ind0_Geo_node1_7_8_14_4T ],dtype=int)
        ind0_Geo_node3_5_4T = np.array([ x for x in ind0_Geo_node3_5_10_12_4T if self.zn_4T[x]==1 ],dtype=int)
        
        (
        self.ind0_Geo_top_4T, 
        self.ind0_Geo_bottom_4T, 
        self.ind0_Geo_left_right_4T, 
        self.ind0_Geo_front_4T, 
        self.ind0_Geo_back_4T,
        self.ind0_Geo_node2_4_6_9_11_13_4T,
        self.ind0_Geo_node1_7_8_14_4T,
        self.ind0_Geo_node1_7_4T,
        self.ind0_Geo_node3_5_10_12_4T,
        self.ind0_Geo_node3_5_4T
        )=(
        ind0_Geo_top_4T, 
        ind0_Geo_bottom_4T, 
        ind0_Geo_left_right_4T, 
        ind0_Geo_front_4T, 
        ind0_Geo_back_4T,
        ind0_Geo_node2_4_6_9_11_13_4T,
        ind0_Geo_node1_7_8_14_4T,
        ind0_Geo_node1_7_4T,
        ind0_Geo_node3_5_10_12_4T,
        ind0_Geo_node3_5_4T
        )
    #########################################################   
    ### function for reading LUT_SoC,LUT_T,LUT_Ri_PerA,LUT_Ci_PerA ####
    #########################################################
    def fun_pre_Thermal(self, pos_neg):          
...
#        global delta_x1_4T, delta_x2_4T, delta_y1_4T, delta_y2_4T, delta_z1_4T, delta_z2_4T, delta_xyz_4T, V_stencil_4T     #for nodes 1~63        
        #-----------------------------------fill in Lamda_4T
        if pos_neg == 'pos':
            Lamda_4T=self.weld_Lamda_pos * np.ones([self.ntotal_4T,6])                                                                                                        #Lamda_4T shape is (63,6). λ term in 6-node stencil
        elif pos_neg == 'neg':
            Lamda_4T=self.weld_Lamda_neg * np.ones([self.ntotal_4T,6])                                                                                                        #Lamda_4T shape is (63,6). λ term in 6-node stencil
        #-----------------------------------fill in RouXc_4T
        if pos_neg == 'pos':
            RouXc_4T=self.weld_rouXc_pos * np.ones([self.ntotal_4T,1])                                                                                                   #RouXc_4T shape is (63,1). ρc term in 6-node stencil
        elif pos_neg == 'neg':
            RouXc_4T=self.weld_rouXc_neg * np.ones([self.ntotal_4T,1])                                                                                                   #RouXc_4T shape is (63,1). ρc term in 6-node stencil
        #-----------------------------------fill in Delta_x1_4T
        ...
        delta_x1_4T=self.weld_L1 * np.ones([self.ntotal_4T])                                                                                      #get δx1 for each node
    
        delta_x2_4T=self.weld_L1 * np.ones([self.ntotal_4T])                                                                                      #get δx1 for each node
        
        delta_y1_4T=np.nan*np.zeros([self.ntotal_4T])                                                                                      #get δy1 for each node
        delta_y1_4T[self.ind0_Geo_node1_7_8_14_4T]=self.weld_W1
        delta_y1_4T[self.ind0_Geo_node2_4_6_9_11_13_4T]=self.weld_W3
        delta_y1_4T[self.ind0_Geo_node3_5_10_12_4T]=self.weld_W2
    
        delta_y2_4T=np.nan*np.zeros([self.ntotal_4T])                                                                                      #get δy1 for each node
        delta_y2_4T[self.ind0_Geo_node1_7_8_14_4T]=self.weld_W1
        delta_y2_4T[self.ind0_Geo_node2_4_6_9_11_13_4T]=self.weld_W3
        delta_y2_4T[self.ind0_Geo_node3_5_10_12_4T]=self.weld_W2

        if self.pos_neg == 'pos':
            delta_z1_4T = (self.weld_T1_pos/self.nz) * np.ones([self.ntotal_4T])                                                                                      #get δz1 for each node
        if self.pos_neg == 'neg':
            delta_z1_4T = (self.weld_T1_neg/self.nz) * np.ones([self.ntotal_4T])                                                                                      #get δz1 for each node
    
        if self.pos_neg == 'pos':
            delta_z2_4T = (self.weld_T1_pos/self.nz) * np.ones([self.ntotal_4T])                                                                                      #get δz1 for each node
        if self.pos_neg == 'neg':
            delta_z2_4T = (self.weld_T1_neg/self.nz) * np.ones([self.ntotal_4T])                                                                                      #get δz1 for each node
...
#        self.plot_Variable_ALL=self.T_record[:,plot_step]-273.15  #variable to be visualized  
        ...
        self.ind0_plot_front = np.tile(self.ind0_plot_front,2)     #this section is to turn 1-line nodes into 2-line nodes
        self.X1=self.zi_4T_ALL[self.ind0_plot_front]
        self.Y1=self.xi_4T_ALL[self.ind0_plot_front]
        self.Y1[:,1] += self.weld_L1
        ...
        self.ind0_plot_back = np.tile(self.ind0_plot_back,2)     #this section is to turn 1-line nodes into 2-line nodes
        self.X2=self.zi_4T_ALL[self.ind0_plot_back]
        self.Y2=self.xi_4T_ALL[self.ind0_plot_back]
        self.Y2[:,1] += self.weld_L1
        ...
        #-----------------------plot left surface
        self.ind0_plot_left=self.ind0_Geo_left_right_4T.reshape(self.nstack,-1).T
        ...
        #-----------------------plot right surface
        self.ind0_plot_right=self.ind0_Geo_left_right_4T.reshape(self.nstack,-1).T
        self.X4=self.zi_4T_ALL[self.ind0_plot_right]
        self.Y4=self.xi_4T_ALL[self.ind0_plot_right]+self.weld_L1
        ...
        #-----------------------plot top surface
        self.ind0_plot_top=self.ind0_Geo_top_4T.reshape(self.nz,1)
        self.ind0_plot_top = np.tile(self.ind0_plot_top,2)
        self.X5=self.zi_4T_ALL[self.ind0_plot_top]
        self.Y5=self.xi_4T_ALL[self.ind0_plot_top]
        self.Y5[:,1] += self.weld_L1
        ...
        #-----------------------plot bottom surface
        self.ind0_plot_bottom=self.ind0_Geo_bottom_4T.reshape(self.nz,1)
        self.ind0_plot_bottom = np.tile(self.ind0_plot_bottom,2)
        self.X6=self.zi_4T_ALL[self.ind0_plot_bottom]
        self.Y6=self.xi_4T_ALL[self.ind0_plot_bottom]
        self.Y6[:,1] += self.weld_L1
